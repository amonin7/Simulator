...
class MasterBalancer(sb.GenericBalancer):
    def __init__(self, max_depth, proc_am, prc_blnc, alive_proc_am, T, S, m, M):
...
class SlaveBalancer(sb.GenericBalancer):
    ...
    def __init__(self, max_depth, proc_am, prc_blnc, T, S, m, M):
        super().__init__(max_depth, proc_am, prc_blnc)
        self.T = T
        self.S = S
        self.M = M
    ...
    def balance(self, state, subs_amount, add_args=None):
        if state == "starting" or state == "receiving" or state == "sent_get_request":
            return "receive", []
